...
class custom_dataset(data.Dataset):
    def __init__(self, img_path, gt_path, scale=0.25, length=512):
        ...
        self.gt_files = [os.path.join(gt_path, gt_file) for gt_file in sorted(os.listdir(gt_path))
                         if not gt_file.startswith(".")]
        self.scale = scale
        self.length = length
        assert len(self.img_files) == len(self.gt_files), "img and gt file count mismatch!"
    ...
    def __getitem__(self, index):
        try:
            ...
            print("Meet wrong data，choose another image randomly.")
